...
var config = {
	...
	skip_vanilla        = false,   # If true, vanilla items are ignored in the output
	...
	add_table       = true, # If true, adds a table before the main grid, showing all the items (eg. see Mod:Invasion)
...
#	statscard += "\n" + indent + "| image  = " + config.image_prefix + image_filename + ".png"
	statscard += "\n" + indent + "| image  = " + tr(item_data.name) + ".png" #MOD_EDIT
